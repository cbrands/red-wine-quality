...
uni_plot <- function(plotvar, plot_label) {
  grid.arrange(
    ggplot(data = wine, aes(x=plotvar)) + geometry + labs(x = plot_label, y = "Number of wines"), 
...
multi_plot <- function(x, y, z, xlabel, ylabel, zlabel) {
  ...
  labels = breaks
  color_values = c("#D55E00", "#E69F00", "#F0E442", "#009E73", "#56B4E9", "#0072B2")
  ggplot(data = wine,aes(x = x, y = y)) +
    geom_point(aes(color = factor(z))) +
    xlab(xlabel) +
    ylab(ylabel) +
    scale_color_manual(name = zlabel, breaks = breaks, labels = labels, values = color_values) 
}
```
...
ggplot(data = wine,aes(x = wine$alcohol, y = wine$volatile.acidity)) +
  ...
  ggtitle("Wine Quality distribution as a function\nof alcohol percentage and volatile acidity") +
